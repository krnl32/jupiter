...
public class KeyboardMovementComponent extends Component {
	...
	public void onUpdate(float dt) {
		...
		if (Input.getInstance().isKeyDown(downKey))
			transform.getTranslation().y -= velocity;

		if (Input.getInstance().isKeyDown(forwardKey))
			transform.getTranslation().z -= velocity;

		if(Input.getInstance().isKeyDown(backwardKey))
			transform.getTranslation().z += velocity;

		if(Input.getInstance().isKeyDown(leftKey))
			transform.getTranslation().x -= velocity;

		if(Input.getInstance().isKeyDown(rightKey))
			transform.getTranslation().x += velocity;
	}
