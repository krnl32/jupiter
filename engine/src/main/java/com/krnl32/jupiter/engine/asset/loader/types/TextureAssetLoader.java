...
public class TextureAssetLoader implements AssetLoader<TextureAsset> {
	...
	public TextureAsset load(AssetMetadata assetMetadata) {
		try {
			...
			return (TextureAsset) assetSerializer.deserialize(FileIO.readFileContentBytes(ProjectContext.getInstance().getAssetDirectory().resolve(assetMetadata.getAssetPath())));
		} catch (Exception e) {
