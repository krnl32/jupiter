...
public class DataScriptComponentSerializer implements ComponentSerializer<ScriptComponent, Map<String, Object>> {
	@Override
	public Map<String, Object> serialize(ScriptComponent component) {
		...
		for (ScriptInstance script : component.scripts) {
			Map<String, Object> scriptData = DataComponentSerializerUtility.serializeScriptInstance(script);
			if (scriptData == null) {
	...
	public ScriptComponent deserialize(Map<String, Object> data, EntityResolver resolver) {
		...
		if (scriptsData != null) {
			for (int i = 0; i < scriptsData.size(); i++) {
				scripts.add(DataComponentSerializerUtility.deserializeScriptInstance((Map<String, Object>) scriptsData.get(i)));
			}
		}
